...
class otp {
	...
	public function otp($no, $argv)
	{
		if(isset($no) and is_numeric($no)) {
			...
			elseif($argv == 'tri') {
				return $this->post('https://registrasi.tri.co.id/daftar/generateOTP','msisdn='.$no);
			}
			...
			elseif($argv == 'myim3') {
				return $this->post('https://myim3.indosatooredoo.com/registration/verifyMsisdn','mobilephone='.$no);
			}
		}
		else {
